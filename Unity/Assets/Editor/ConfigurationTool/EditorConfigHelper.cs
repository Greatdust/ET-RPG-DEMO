...
public static class EditorEnterBattleHelper
{
    private static int unitTypeId = 1001;
    private static int minNum = 0;
    private static int maxNum = 0;
    public static void BattleConfig(EnterBattleEventData enterBattleEventData)
    {
        enterBattleEventData.sceneName = EditorGUILayout.DelayedTextField("战斗场景名:", enterBattleEventData.sceneName,GUILayout.Height(20));

        enterBattleEventData.usePlayerTeam = EditorGUILayout.ToggleLeft("勾选后使用玩家的队伍", enterBattleEventData.usePlayerTeam, EditorGUIHelper.GetGUIStyle(Color.blue), GUILayout.Height(25));
        EditorGUILayout.LabelField("若不勾选,则队伍内我方角色全部由下面的支援角色列表组成");
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        EditorGUILayout.BeginHorizontal(GUILayout.Height(25));
        EditorGUILayout.LabelField("输入一个单位的TypeId", GUILayout.Width(200), GUILayout.Height(25));
        unitTypeId = EditorGUILayout.DelayedIntField(unitTypeId);
        EditorGUILayout.EndHorizontal();
        using (new EditorGUILayout.HorizontalScope(GUILayout.Width(400)))
        {
            EditorGUILayout.LabelField("如果是敌方单位,还需要输入数量", GUILayout.Width(200), GUILayout.Height(25));
            EditorGUILayout.LabelField("最小:", GUILayout.Width(30), GUILayout.Height(25));
            minNum = EditorGUILayout.DelayedIntField(minNum,GUILayout.Width(50));
            EditorGUILayout.LabelField("最大:", GUILayout.Width(30), GUILayout.Height(25));
            maxNum = EditorGUILayout.DelayedIntField(maxNum,GUILayout.Width(50));
        }
        EditorGUILayout.BeginHorizontal(GUILayout.Height(25));
        if (GUILayout.Button("加入我方队伍", GUILayout.Width(150)))
        {
            if (unitTypeId <= 1000)
            {
                return;
            }
            if (enterBattleEventData.unitInOurTeam == null)
                enterBattleEventData.unitInOurTeam = new List<int>();
            if (enterBattleEventData.unitInOurTeam.Count >= 3)
            {
                EditorUtility.DisplayDialog("错误", "我方支援角色数量不得超过3个!", "确定");
                return;
            }
            enterBattleEventData.unitInOurTeam.Add(unitTypeId);

        }
        if (GUILayout.Button("加入敌方队伍", GUILayout.Width(150)))
        {
            if (unitTypeId <= 1000)
            {
                return;
            }
            if (enterBattleEventData.unitInEnemyTeam == null)
                enterBattleEventData.unitInEnemyTeam = new List<EnterBattleEventData.EnemyConfig>();
            int totalNum = 0;
            foreach (var v in enterBattleEventData.unitInEnemyTeam)
            {
                totalNum += v.minNum;
            }
            ...
            if (totalNum >= 6)
            {
                EditorUtility.DisplayDialog("错误", "敌方角色数量不得超过6个!", "确定");
                return;
            }


            enterBattleEventData.unitInEnemyTeam.Add(new EnterBattleEventData.EnemyConfig() { unitTypeId = unitTypeId, minNum = minNum, maxNum = maxNum });
        }
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.EndVertical();
        enterBattleEventData.victoryConditionType = (VictoryConditionType)EditorGUILayout.EnumPopup("胜利条件:", enterBattleEventData.victoryConditionType);
        enterBattleEventData.failureConditionType = (FailureConditionType)EditorGUILayout.EnumPopup("失败条件:", enterBattleEventData.failureConditionType);
        if (enterBattleEventData.unitInOurTeam != null && enterBattleEventData.unitInOurTeam.Count > 0)
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("我方支援角色列表", EditorGUIHelper.GetGUIStyle(Color.blue));
            for (int i = 0; i < enterBattleEventData.unitInOurTeam.Count; i++)
            {
                var v = enterBattleEventData.unitInOurTeam[i];
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("支援角色TypeId:\t" + v.ToString(), GUILayout.Width(200), GUILayout.Height(25));
                if (GUILayout.Button("删除角色", GUILayout.Width(150)))
                {
                    enterBattleEventData.unitInOurTeam.Remove(v);
                }
                EditorGUILayout.EndHorizontal();
            }
            EditorGUILayout.EndVertical();
        }
        if (enterBattleEventData.unitInEnemyTeam != null && enterBattleEventData.unitInEnemyTeam.Count > 0)
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("敌方角色列表", EditorGUIHelper.GetGUIStyle(Color.blue));
            for (int i = 0; i < enterBattleEventData.unitInEnemyTeam.Count; i++)
            {
                var v = enterBattleEventData.unitInEnemyTeam[i];
                EditorGUILayout.BeginHorizontal(EditorStyles.helpBox);
                EditorGUILayout.LabelField(string.Format("敌方角色TypeId:\t{0}", v.unitTypeId.ToString()), GUILayout.Width(200), GUILayout.Height(25));
                using (new EditorGUILayout.VerticalScope())
                {
                    v.minNum = EditorGUILayout.IntField("最小数量", v.minNum);
                    v.maxNum = EditorGUILayout.IntField("最大数量", v.maxNum);
                }
                EditorGUILayout.EndHorizontal();
                enterBattleEventData.unitInEnemyTeam[i] = v;
                if (GUILayout.Button("删除角色", GUILayout.Width(150)))
                {
                    enterBattleEventData.unitInEnemyTeam.Remove(v);
                }
            }
            EditorGUILayout.EndVertical();
        }
