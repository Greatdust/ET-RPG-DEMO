...
namespace ETModel
{
	...
	public class NumericChangeEvent_NotifyWatcher: AEvent<NumericType, long, float>
	{
		...
		{
            NumericComponent numericComponent = UnitComponent.Instance.Get(destUnitId).GetComponent<NumericComponent>();
            ...
            if (property_Die.Get()) return; //角色已经死亡,不再受到任何属性影响
            switch (nt)
            {
                case NumericType.HP:
                ...
                case NumericType.MP_LoseValue:
                    ...
                    break;
                default:
                    value = numericComponent.GetAsInt(nt) + Mathf.RoundToInt(updateValue);
