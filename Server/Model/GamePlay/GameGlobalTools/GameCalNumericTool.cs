...
public static class GameCalNumericTool
{
    ...
    public static bool CalFinalDamage(long sourceUnitId,long destUnitId,DamageData skillDamageValue)
    {
        try
        {
            Unit dest = Game.Scene.GetComponent<UnitComponent>().Get(destUnitId);
            Unit source = Game.Scene.GetComponent<UnitComponent>().Get(sourceUnitId);
            CharacterStateComponent unitState = dest.GetComponent<CharacterStateComponent>();
            ...
            NumericComponent sourceUnitNumericCom = source.GetComponent<NumericComponent>();
            NumericComponent destUnitNumericCom = dest.GetComponent<NumericComponent>();
            int rateCharge = 0;
            //命中判定
